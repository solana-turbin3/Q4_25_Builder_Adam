...
pub struct ResolveBet<'info> {
    ...
    #[account(
        address = sysvar::instructions::ID @ DiceError::InstructionnSysvarNotFound
    )]
    ...
    pub fn resolve_bet(&mut self, sig: &[u8], bumps: &ResolveBetBumps) -> Result<()> {
        ...
        if self.bet.roll > roll {
            ...
            let payout = (self.bet.amount as u128)
                .checked_mul(bps.checked_sub(HOUSE_EDGE_BPS as u128).ok_or(DiceError::Overflow)?)
                .ok_or(DiceError::Overflow)?
                .checked_div(self.bet.roll as u128).ok_or(DiceError::Overflow)?
                .checked_div(bps).ok_or(DiceError::Overflow)? as u64;
