...
function bottomUpTransformerVisitor(transformers, options = {}) {
  const transformerFunctions = transformers.map((transformer) => {
    ...
    return (node, stack2) => getConjunctiveNodeSelectorFunction(transformer.select)(stack2.getPath(REGISTERED_NODE_KINDS5)) ? transformer.transform(node, stack2) : node;
  });
...
import { REGISTERED_NODE_KINDS as REGISTERED_NODE_KINDS7 } from "@codama/nodes";
function topDownTransformerVisitor(transformers, options = {}) {
  const transformerFunctions = transformers.map((transformer) => {
    if (typeof transformer === "function") return transformer;
    return (node, stack2) => getConjunctiveNodeSelectorFunction(transformer.select)(stack2.getPath(REGISTERED_NODE_KINDS7)) ? transformer.transform(node, stack2) : node;
  });
  const stack = options.stack ?? new NodeStack();
  return pipe(
    identityVisitor(options),
...
import { REGISTERED_NODE_KINDS as REGISTERED_NODE_KINDS8 } from "@codama/nodes";
function extendVisitor(visitor, overrides) {
  const registeredVisitFunctions = REGISTERED_NODE_KINDS8.map(getVisitFunctionName);
...
// src/mergeVisitor.ts
import { getAllPrograms, REGISTERED_NODE_KINDS as REGISTERED_NODE_KINDS9 } from "@codama/nodes";
function mergeVisitor(leafValue, merge, options = {}) {
  const keys = options.keys ?? REGISTERED_NODE_KINDS9;
  ...
  return pipe(
    ...
    (v) => extendVisitor(v, {
      ...
      visitConstantValue(node, { self }) {
        ...
        if (isNode2(node.value, "bytesValueNode") && node.value.encoding === "base16") {
          return Math.ceil(node.value.data.length / 2);
        }
        if (isNode2(node.type, "stringTypeNode") && node.type.encoding === "base16" && isNode2(node.value, "stringValueNode")) {
...
function getArrayLikeSize(count, innerSize, self) {
  if (innerSize === 0 && isNode2(count, "prefixedCountNode")) return visit(count.prefix, self);
  if (innerSize === 0) return 0;
  if (!isNode2(count, "fixedCountNode")) return null;
...
import { isNode as isNode3, isScalarEnum as isScalarEnum2, REGISTERED_TYPE_NODE_KINDS as REGISTERED_TYPE_NODE_KINDS2 } from "@codama/nodes";
  ...
  return pipe(
    ...
    (v) => extendVisitor(v, {
      ...
      visitConstantValue(node, { self }) {
        const typeSize = visit(node.type, self);
        if (typeSize !== null) return typeSize;
        if (isNode3(node.value, "bytesValueNode") && node.value.encoding === "base16") {
          return Math.ceil(node.value.data.length / 2);
        }
        if (isNode3(node.type, "stringTypeNode") && node.type.encoding === "base16" && isNode3(node.value, "stringValueNode")) {
          return Math.ceil(node.value.string.length / 2);
        }
        return null;
      },
      ...
      visitEnumType(node, { self }) {
        const prefix = visit(node.size, self);
        if (prefix === null) return null;
        if (isScalarEnum2(node)) return prefix;
        const variantSizes = node.variants.map((v2) => visit(v2, self));
...
function getArrayLikeSize2(count, innerSize, self) {
  if (innerSize === 0 && isNode3(count, "prefixedCountNode")) return visit(count.prefix, self);
  if (innerSize === 0) return 0;
  if (!isNode3(count, "fixedCountNode")) return null;
  if (count.value === 0) return 0;
  return innerSize !== null ? innerSize * count.value : null;
}
...
function getResolvedInstructionInputsVisitor(options = {}) {
  ...
  function resolveInstructionInput(instruction, input) {
    if (isNode4(input, "instructionAccountNode") && visitedAccounts.has(input.name) || isNode4(input, "instructionArgumentNode") && visitedArgs.has(input.name)) {
      return;
    }
  ...
  function resolveInstructionAccount(instruction, account) {
    ...
    const localResolved = {
      ...
      isPda: getAllInstructionArguments(instruction).some(
        (argument) => isNode4(argument.defaultValue, "accountBumpValueNode") && argument.defaultValue.name === account.name
      ),
    ...
    switch (localResolved.defaultValue?.kind) {
      ...
      case "pdaValueNode":
        ...
        seeds.forEach((seed) => {
          if (!isNode4(seed.value, "accountValueNode")) return;
  ...
  function resolveInstructionDependencies(instruction, parent, dependencies) {
    dependencies.forEach((dependency) => {
      let input = null;
      if (isNode4(dependency, "accountValueNode")) {
      ...
      } else if (isNode4(dependency, "argumentValueNode")) {
  ...
  return singleNodeVisitor("instructionNode", (node) => {
    ...
    const inputs = [
      ...
      ...node.arguments.filter((a) => {
        ...
        return a.defaultValue && !isNode4(a.defaultValue, VALUE_NODES2);
      }),
...
function getInstructionDependencies(input) {
  if (isNode4(input, "instructionNode")) {
  ...
  if (isNode4(input.defaultValue, ["accountValueNode", "accountBumpValueNode"])) {
    return [accountValueNode(input.defaultValue.name)];
  }
  if (isNode4(input.defaultValue, ["argumentValueNode"])) {
    return [argumentValueNode(input.defaultValue.name)];
  }
  if (isNode4(input.defaultValue, "pdaValueNode")) {
    const dependencies = /* @__PURE__ */ new Map();
    input.defaultValue.seeds.forEach((seed) => {
      if (isNode4(seed.value, ["accountValueNode", "argumentValueNode"])) {
        dependencies.set(seed.value.name, { ...seed.value });
      }
    });
    return [...dependencies.values()];
  }
  if (isNode4(input.defaultValue, "resolverValueNode")) {
    return input.defaultValue.dependsOn ?? [];
  }
  if (isNode4(input.defaultValue, "conditionalValueNode")) {
    return deduplicateInstructionDependencies([
...
var LinkableDictionary = class {
  ...
  getOrCreateProgramDictionary(linkablePath) {
    const linkableNode = getLastNodeFromPath(linkablePath);
    const programNode2 = isNode5(linkableNode, "programNode") ? linkableNode : findProgramNodeFromPath(linkablePath);
  ...
  getOrCreateInstructionDictionary(programDictionary, linkablePath) {
    const linkableNode = getLastNodeFromPath(linkablePath);
    const instructionNode2 = isNode5(linkableNode, "instructionNode") ? linkableNode : findInstructionNodeFromPath(linkablePath);
  ...
  getProgramDictionary(linkPath) {
    ...
    if (isNode5(linkNode, "programLinkNode")) {
  ...
  getInstructionDictionary(programDictionary, linkPath) {
    ...
    if (isNode5(linkNode, "instructionLinkNode")) {
...
import { isNode as isNode6 } from "@codama/nodes";
...
function getRecordLinkablesVisitor(linkables) {
  ...
  return pipe(
    voidVisitor(),
    (v) => interceptVisitor(v, (node, next) => {
      if (isNode6(node, LINKABLE_NODES)) {
