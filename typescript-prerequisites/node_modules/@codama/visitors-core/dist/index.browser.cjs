...
__export(index_exports, {
  ...
  getMaxByteSizeVisitor: () => getMaxByteSizeVisitor,
...
function bottomUpTransformerVisitor(transformers, options = {}) {
  const transformerFunctions = transformers.map((transformer) => {
    ...
    return (node, stack2) => getConjunctiveNodeSelectorFunction(transformer.select)(stack2.getPath(import_nodes7.REGISTERED_NODE_KINDS)) ? transformer.transform(node, stack2) : node;
  });
...
function topDownTransformerVisitor(transformers, options = {}) {
  const transformerFunctions = transformers.map((transformer) => {
    if (typeof transformer === "function") return transformer;
    return (node, stack2) => getConjunctiveNodeSelectorFunction(transformer.select)(stack2.getPath(import_nodes9.REGISTERED_NODE_KINDS)) ? transformer.transform(node, stack2) : node;
  });
  const stack = options.stack ?? new NodeStack();
  return pipe(
    identityVisitor(options),
...
function extendVisitor(visitor, overrides) {
  const registeredVisitFunctions = import_nodes10.REGISTERED_NODE_KINDS.map(getVisitFunctionName);
...
function mergeVisitor(leafValue, merge, options = {}) {
  const keys = options.keys ?? import_nodes11.REGISTERED_NODE_KINDS;
  const visitor = staticVisitor(leafValue, { keys });
  const visit2 = (v) => (node) => keys.includes(node.kind) ? [visit(node, v)] : [];
  if (keys.includes("rootNode")) {
    visitor.visitRoot = function visitRoot(node) {
      return merge(node, (0, import_nodes11.getAllPrograms)(node).flatMap(visit2(this)));
    };
  }
  if (keys.includes("programNode")) {
    visitor.visitProgram = function visitProgram(node) {
...
function getByteSizeVisitor(linkables, options = {}) {
  ...
  const baseVisitor = mergeVisitor(
    ...
    {
      keys: [
        ...import_nodes12.REGISTERED_TYPE_NODE_KINDS,
        "accountNode",
        "constantValueNode",
        "definedTypeLinkNode",
        "definedTypeNode",
  ...
  return pipe(
    ...
    (v) => extendVisitor(v, {
      ...
      visitConstantValue(node, { self }) {
        ...
        if ((0, import_nodes12.isNode)(node.value, "bytesValueNode") && node.value.encoding === "base16") {
          return Math.ceil(node.value.data.length / 2);
        }
        if ((0, import_nodes12.isNode)(node.type, "stringTypeNode") && node.type.encoding === "base16" && (0, import_nodes12.isNode)(node.value, "stringValueNode")) {
      ...
      visitEnumType(node, { self }) {
        ...
        if (prefix === null) return null;
        if ((0, import_nodes12.isScalarEnum)(node)) return prefix;
...
function getArrayLikeSize(count, innerSize, self) {
  if (innerSize === 0 && (0, import_nodes12.isNode)(count, "prefixedCountNode")) return visit(count.prefix, self);
  if (innerSize === 0) return 0;
  if (!(0, import_nodes12.isNode)(count, "fixedCountNode")) return null;
...
function getMaxByteSizeVisitor(linkables, options = {}) {
  ...
  const baseVisitor = mergeVisitor(
    ...
    {
      keys: [
        ...import_nodes13.REGISTERED_TYPE_NODE_KINDS,
        "accountNode",
        "constantValueNode",
        "definedTypeLinkNode",
        "definedTypeNode",
  ...
  return pipe(
    ...
    (v) => extendVisitor(v, {
      ...
      visitConstantValue(node, { self }) {
        const typeSize = visit(node.type, self);
        if (typeSize !== null) return typeSize;
        if ((0, import_nodes13.isNode)(node.value, "bytesValueNode") && node.value.encoding === "base16") {
          return Math.ceil(node.value.data.length / 2);
        }
        if ((0, import_nodes13.isNode)(node.type, "stringTypeNode") && node.type.encoding === "base16" && (0, import_nodes13.isNode)(node.value, "stringValueNode")) {
          return Math.ceil(node.value.string.length / 2);
        }
        return null;
      },
      ...
      visitEnumType(node, { self }) {
        const prefix = visit(node.size, self);
        if (prefix === null) return null;
        if ((0, import_nodes13.isScalarEnum)(node)) return prefix;
        const variantSizes = node.variants.map((v2) => visit(v2, self));
...
function getArrayLikeSize2(count, innerSize, self) {
  if (innerSize === 0 && (0, import_nodes13.isNode)(count, "prefixedCountNode")) return visit(count.prefix, self);
  if (innerSize === 0) return 0;
  if (!(0, import_nodes13.isNode)(count, "fixedCountNode")) return null;
  if (count.value === 0) return 0;
  return innerSize !== null ? innerSize * count.value : null;
}
...
function getResolvedInstructionInputsVisitor(options = {}) {
  ...
  function resolveInstructionInput(instruction, input) {
    if ((0, import_nodes14.isNode)(input, "instructionAccountNode") && visitedAccounts.has(input.name) || (0, import_nodes14.isNode)(input, "instructionArgumentNode") && visitedArgs.has(input.name)) {
      return;
    }
  ...
  function resolveInstructionAccount(instruction, account) {
    ...
    const localResolved = {
      ...account,
      dependsOn,
      isPda: (0, import_nodes14.getAllInstructionArguments)(instruction).some(
        (argument) => (0, import_nodes14.isNode)(argument.defaultValue, "accountBumpValueNode") && argument.defaultValue.name === account.name
      ),
    ...
    switch (localResolved.defaultValue?.kind) {
      ...
      case "pdaValueNode":
        ...
        seeds.forEach((seed) => {
          if (!(0, import_nodes14.isNode)(seed.value, "accountValueNode")) return;
  ...
  function resolveInstructionDependencies(instruction, parent, dependencies) {
    dependencies.forEach((dependency) => {
      let input = null;
      if ((0, import_nodes14.isNode)(dependency, "accountValueNode")) {
      ...
      } else if ((0, import_nodes14.isNode)(dependency, "argumentValueNode")) {
        const dependencyArgument = (0, import_nodes14.getAllInstructionArguments)(instruction).find(
  ...
  return singleNodeVisitor("instructionNode", (node) => {
    ...
    const inputs = [
      ...
      ...node.arguments.filter((a) => {
        ...
        return a.defaultValue && !(0, import_nodes14.isNode)(a.defaultValue, import_nodes14.VALUE_NODES);
      }),
...
function getInstructionDependencies(input) {
  if ((0, import_nodes14.isNode)(input, "instructionNode")) {
  ...
  if ((0, import_nodes14.isNode)(input.defaultValue, ["accountValueNode", "accountBumpValueNode"])) {
    return [(0, import_nodes14.accountValueNode)(input.defaultValue.name)];
  }
  if ((0, import_nodes14.isNode)(input.defaultValue, ["argumentValueNode"])) {
    return [(0, import_nodes14.argumentValueNode)(input.defaultValue.name)];
  }
  if ((0, import_nodes14.isNode)(input.defaultValue, "pdaValueNode")) {
    const dependencies = /* @__PURE__ */ new Map();
    input.defaultValue.seeds.forEach((seed) => {
      if ((0, import_nodes14.isNode)(seed.value, ["accountValueNode", "argumentValueNode"])) {
        dependencies.set(seed.value.name, { ...seed.value });
      }
    });
    return [...dependencies.values()];
  }
  if ((0, import_nodes14.isNode)(input.defaultValue, "resolverValueNode")) {
    return input.defaultValue.dependsOn ?? [];
  }
  if ((0, import_nodes14.isNode)(input.defaultValue, "conditionalValueNode")) {
    return deduplicateInstructionDependencies([
...
var LinkableDictionary = class {
  ...
  getPath(linkPath) {
    ...
    if ((0, import_nodes15.isNode)(linkNode, "accountLinkNode")) {
      return programDictionary.accounts.get(linkNode.name);
    } else if ((0, import_nodes15.isNode)(linkNode, "definedTypeLinkNode")) {
      return programDictionary.definedTypes.get(linkNode.name);
    } else if ((0, import_nodes15.isNode)(linkNode, "instructionAccountLinkNode")) {
      return instructionDictionary?.accounts.get(linkNode.name);
    } else if ((0, import_nodes15.isNode)(linkNode, "instructionArgumentLinkNode")) {
      return instructionDictionary?.arguments.get(linkNode.name);
    } else if ((0, import_nodes15.isNode)(linkNode, "instructionLinkNode")) {
      return instructionDictionary?.instruction;
    } else if ((0, import_nodes15.isNode)(linkNode, "pdaLinkNode")) {
      return programDictionary.pdas.get(linkNode.name);
    } else if ((0, import_nodes15.isNode)(linkNode, "programLinkNode")) {
  ...
  getOrCreateProgramDictionary(linkablePath) {
    const linkableNode = getLastNodeFromPath(linkablePath);
    const programNode2 = (0, import_nodes15.isNode)(linkableNode, "programNode") ? linkableNode : findProgramNodeFromPath(linkablePath);
  ...
  getOrCreateInstructionDictionary(programDictionary, linkablePath) {
    const linkableNode = getLastNodeFromPath(linkablePath);
    const instructionNode2 = (0, import_nodes15.isNode)(linkableNode, "instructionNode") ? linkableNode : findInstructionNodeFromPath(linkablePath);
  ...
  getProgramDictionary(linkPath) {
    ...
    if ((0, import_nodes15.isNode)(linkNode, "programLinkNode")) {
  ...
  getInstructionDictionary(programDictionary, linkPath) {
    ...
    if ((0, import_nodes15.isNode)(linkNode, "instructionLinkNode")) {
...
function getRecordLinkablesVisitor(linkables) {
  ...
  return pipe(
    voidVisitor(),
    (v) => interceptVisitor(v, (node, next) => {
      if ((0, import_nodes16.isNode)(node, LINKABLE_NODES)) {
